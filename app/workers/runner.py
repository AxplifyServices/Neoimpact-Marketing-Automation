...
def _best_channel_spec() -> Tuple[Callable[[], Any], Callable[[], None], Callable[[], Dict[str, Any]], Callable[[Dict[str, Any]], bool]]:
    from app.best_channel.worker import (
        best_channel_worker_status,
        start_best_channel_worker,
        stop_best_channel_worker,
    )
    return (
        start_best_channel_worker,
        stop_best_channel_worker,
...
def _commercial_pressure_spec() -> Tuple[Callable[[], Any], Callable[[], None], Callable[[], Dict[str, Any]], Callable[[Dict[str, Any]], bool]]:
    from app.commercial_pressure.worker import (
        commercial_pressure_worker_status,
        start_commercial_pressure_worker,
        stop_commercial_pressure_worker,
    )
    return (
        start_commercial_pressure_worker,
        stop_commercial_pressure_worker,
...
def _product_scoring_spec() -> Tuple[Callable[[], Any], Callable[[], None], Callable[[], Dict[str, Any]], Callable[[Dict[str, Any]], bool]]:
    from app.product_scoring.worker import (
        product_scoring_worker_status,
        start_product_scoring_worker,
        stop_product_scoring_worker,
    )
    return (
        start_product_scoring_worker,
        stop_product_scoring_worker,
...
def _spec(role: str):
    specs = {
        ...
        "best_channel": _best_channel_spec,
        "commercial_pressure": _commercial_pressure_spec,
        "product_scoring": _product_scoring_spec,
    }
...
def main() -> int:
    ...
    if not role:
        print("usage: python -m app.workers.runner <campaign|outbound|inbound|batch|segmentation|attrition|digital_engagement|best_channel|commercial_pressure|product_scoring>", file=sys.stderr)
        return 2
