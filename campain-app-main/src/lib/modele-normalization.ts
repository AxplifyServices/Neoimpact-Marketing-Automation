...
function parseBackendCondition(oldCond: any, blockId: string | number, idx: number): BlockCondition {
  ...
  if (oldCond.condition_type === 'client_filter') {
    condition.type = 'client_filter';
    condition.column = oldCond.column || oldCond.field?.replace('client.', '');
    condition.min = oldCond.min || '';
    condition.max = oldCond.max || '';
    condition.values = Array.isArray(oldCond.values) ? oldCond.values : [];
    return condition;
  }
