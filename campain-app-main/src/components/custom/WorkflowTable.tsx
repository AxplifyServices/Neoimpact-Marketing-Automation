...
function formatCondition(cond: Block['objectiveConditions'][number]): string {
  if (cond.type === 'days_since_last' && cond.daysSinceLastAction !== undefined) {
    return `${cond.daysSinceLastAction}j depuis dernière action`;
  }
  if (cond.type === 'flag_resultat' && cond.flagResultat) {
    return `Flag résultats ${cond.operator || '='} ${cond.flagResultat}`;
  }
  if (cond.type === 'counter' && cond.counterValue !== undefined) {
    return `Compteur ${cond.operator || '='} ${cond.counterValue}`;
  }
  if (cond.type === 'campaign_field' && cond.campaignField) {
    return `${cond.campaignField} ${cond.operator || '='} ${cond.campaignFieldValue ?? ''}`;
  }
  if (cond.type === 'client_field' && cond.clientField) {
    return `${cond.clientField} ${cond.operator || '='} ${cond.clientFieldValue ?? ''}`;
  }
  if (cond.type === 'client_filter' && cond.column) {
    ...
    if (values.length > 0) return `${cond.column} = ${values.join(', ')}`;
    if (min && max) return `${cond.column} ∈ [${min}, ${max}]`;
    if (min) return `${cond.column} >= ${min}`;
    if (max) return `${cond.column} <= ${max}`;
    return cond.column;
  }
  return '';
}
                  ...
                  {block.parents.length > 0 ? (
                    <div className="flex flex-col gap-1">
                      {block.parents.map(parentId => {
                        ...
                        const parentNum = getBlockDisplayNumber(parentId);
                        return (
                          <div key={parentId} className="text-xs">
                            {block.parents.length > 1 && (
                              <span className="text-gray-400 font-medium">#{parentNum}: </span>
                            )}
                            {conditions.map((cond, idx) => (
                              <span key={cond.id || idx}>
                                {idx > 0 && <span className="text-gray-400"> &amp; </span>}
                                <span className="text-gray-700">{formatCondition(cond)}</span>
                              </span>
                            ))}
                          </div>
                        );
                      })}
                      {Object.values(block.conditionsByParent).every(c => c.length === 0) && (
