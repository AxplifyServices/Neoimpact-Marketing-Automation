...
export default function CreateModelePage() {
  ...
  useEffect(() => {
    ...
    if (editPayload.ui_positions && typeof editPayload.ui_positions === 'object') {
      bm.setLayout(editPayload.ui_positions, { recordHistory: false });
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      ...
      ui_positions: bm.layout ?? undefined,
    });
  };
              ...
              <TabsContent value="visual">
                {bm.blocks.length === 0 ? (
                  ...
                  <Suspense fallback={<div className="h-[420px] rounded-lg border border-gray-200 bg-white flex items-center justify-center text-sm text-gray-500">Chargement du workflow…</div>}><WorkflowPreview
                    ...
                    layout={bm.layout}
                    onLayoutChange={bm.setLayout}
                    onClearLayout={bm.clearLayout}
                    onEdgeBendPointsChange={bm.setEdgeBendPoints}
                    onEdgeHandlesChange={bm.setEdgeHandles}
                    showHeader={false}
                    showFrame={false}
