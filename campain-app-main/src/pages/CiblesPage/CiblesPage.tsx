import { Database, FileText, Calendar, MoreVertical, Edit, Trash2, Eye, Plus, Lock, Copy, Target, CheckCircle2, XCircle, MinusCircle } from 'lucide-react';
...
const readObjectif = (filtre: CibleData['filtre']) => {
  const f = typeof filtre === 'object' && filtre !== null ? filtre : null;
  return f?.[OBJECTIF_KEY] as { mode?: string; values?: unknown[] } | undefined;
};
...
export default function CiblesPage() {
  ...
  const columns = useMemo<ColumnDef<CibleData>[]>(
    () => [
      ...
      },
      {
        id: 'objectif_mode',
        accessorFn: (row) => {
          const mode = readObjectif(row.filtre)?.mode;
          return isObjectifMode(mode) ? mode : 'none';
        },
        meta: { defaultHidden: true },
        filterFn: (row, id, value: string[]) => value.includes(row.getValue(id) as string),
      },
      {
        id: 'objectif_campagnes',
        accessorFn: (row) => {
          const values = readObjectif(row.filtre)?.values;
  ...
  const objectifFilterOptions = useMemo(() => [
    { label: 'Abouti', value: 'atteint', icon: CheckCircle2 },
    { label: 'Non abouti', value: 'non_atteint', icon: XCircle },
    { label: 'Sans objectif', value: 'none', icon: MinusCircle },
  ], []);
