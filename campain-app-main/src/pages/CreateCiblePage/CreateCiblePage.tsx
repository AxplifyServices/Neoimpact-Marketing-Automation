import { ArrowLeft, Plus, Trash2, Database, FileText, Target, X } from 'lucide-react';
...
export default function CreateCiblePage() {
  ...
  const [objectifMode, setObjectifMode] = useState<ObjectifMode | null>(null);
  const [objectifCampaigns, setObjectifCampaigns] = useState<string[]>([]);
  ...
  const campaignOptions = useMemo(
    () =>
      campaignList.map((c) => ({
        value: c.id_campagne,
        label: `${c.id_campagne} — ${c.nom_campagne}`,
      })),
    [campaignList]
  );
    ...
    if (isEditing || isDuplicating) {
      ...
      try {
        ...
        Object.entries(parsedFiltre).forEach(([column, value]: [string, any]) => {
          if (column === OBJECTIF_KEY) {
            if (isObjectifMode(value?.mode)) {
              setObjectifMode(value.mode);
            }
            const values = Array.isArray(value?.values) ? value.values.map(String) : [];
            setObjectifCampaigns(values);
            return;
          }
  ...
  const handleSubmit = () => {
    ...
    const hasObjectifMode = !!objectifMode;
    const hasObjectifCampaigns = objectifCampaigns.length > 0;
    const objectifPartial = hasObjectifMode !== hasObjectifCampaigns;
    if (objectifPartial) {
      setToast({
        isOpen: true,
        title: 'Erreur',
        message: 'Sélectionnez un mode (Abouti / Non abouti) et au moins une campagne pour l\'objectif',
        type: 'error',
      });
      return;
    }
    ...
    if (objectifMode && objectifCampaigns.length > 0) {
      filtre[OBJECTIF_KEY] = { mode: objectifMode, values: objectifCampaigns };
    }
            ...
            <div className="border border-gray-200 rounded-lg p-5 bg-white">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-gray-900">Objectif de campagnes</h3>
                </div>
                {(objectifMode || objectifCampaigns.length > 0) && (
                  <button
                    type="button"
                    onClick={() => {
                      setObjectifMode(null);
                      setObjectifCampaigns([]);
                    }}
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-5">
                <div>
                  ...
                  <div className="mt-2 inline-flex p-1 bg-gray-100 rounded-md">
                    {OBJECTIF_MODES.map((mode) => {
                      const active = objectifMode === mode;
                      return (
                        <button
                          key={mode}
                          type="button"
                          onClick={() => setObjectifMode(active ? null : mode)}
                ...
                <div className="flex-1 min-w-0">
                  <Label className="block text-xs text-gray-600">Campagnes</Label>
                  <div className="mt-2">
                    <MultiSelect
                      options={campaignOptions}
                      selected={objectifCampaigns}
                      onChange={setObjectifCampaigns}
                      placeholder="Sélectionner les campagnes..."
                    />
                  </div>
                </div>
              </div>
