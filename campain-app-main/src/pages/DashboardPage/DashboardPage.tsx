...
export default function DashboardPage() {
  ...
  const { data: modele } = useQuery({
    ...
    queryFn: () => apiClient.request<{ liste_action?: string; blocks?: unknown; ui_positions?: string }>(modelesApi.findById(modeleId!)),
    enabled: !!modeleId,
  });
  ...
  const workflowLayout = useMemo<WorkflowLayout | null>(() => {
    if (!modele?.ui_positions) return null;
    try {
      const parsed = typeof modele.ui_positions === 'string'
        ? JSON.parse(modele.ui_positions)
        : modele.ui_positions;
      return parsed?.nodePositions ? parsed : null;
    } catch { return null; }
  }, [modele?.ui_positions]);
