import { Mail, Phone, Users, MessageSquare, RefreshCw, MapPin } from 'lucide-react';
import { useMemo, useState, useEffect } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { terrainQueuesApi } from '@/lib/api/definitions/terrain-queues.api';
import { batchApi } from '@/lib/api/definitions/batch.api';
import { getApiClient } from '@/lib/api/api-client';
import type { QueueContact, QueueType } from '@/types/campaign.types';
import { normalizeKey, formatKeyLabel, getKeyRank } from '@/lib/queue-contact-utils';
...
export default function TerrainPage() {
  ...
  const { data: gestionnairesData } = useQuery<{ gestionnaires: string[] }>({
    queryKey: ['terrain-gestionnaires', selectedQueue],
    queryFn: () => apiClient.request<{ gestionnaires: string[] }>(terrainQueuesApi.getGestionnaires(selectedQueue!)),
  ...
  const {
    ...
    queryKey: ['terrain-queue-next', selectedQueue, selectedCampaign, selectedGestionnaire],
    queryFn: () =>
      selectedQueue
        ? apiClient.request<QueueContact>(terrainQueuesApi.getNext(selectedQueue, selectedCampaign, selectedGestionnaire))
        : null,
    enabled: !!selectedQueue,
    retry: false,
  });
  ...
  const applyResultMutation = useMutation({
    ...
    onSuccess: () => {
      setShowPrevious(false);
      queryClient.invalidateQueries({ queryKey: ['terrain-queue-next', selectedQueue, selectedCampaign, selectedGestionnaire] });
      setToast({
        isOpen: true,
        title: 'Succes',
    ...
    onError: (err: any) => {
      setToast({
        isOpen: true,
        title: 'Erreur',
        message: err?.message || 'Impossible d\'appliquer le resultat',
        type: 'error',
      });
    },
  });
  ...
  const runBatchMutation = useMutation({
    ...
    onError: (err: any) => {
      setToast({
        isOpen: true,
        title: 'Erreur',
        message: err?.message || 'Impossible de lancer le batch',
        type: 'error',
      });
    },
  });
  ...
  const selectedQueueBlock = QUEUE_BLOCKS.find((block) => block.id === selectedQueue);
  const selectedQueueDotColor = selectedQueueBlock?.iconColor
    ? selectedQueueBlock.iconColor.replace('text-', 'bg-')
    : 'bg-gray-300';

  const rowEntries = useMemo(
    () => (displayContact?.row ? Object.entries(displayContact.row) : []),
    [displayContact?.row]
  );
  ...
  const contextEntries = useMemo(
    () => (displayContact?.context ? Object.entries(displayContact.context) : []),
  ...
  const contactSummary = useMemo(() => {
    ...
    const address = findValue(key => key.includes('adresse') || key.includes('address'));
    ...
    return { name, phone, email, address, campaignId, accountId };
  }, [rowEntries]);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 sm:mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900">Action Terrain</h1>
          </div>
          <p className="text-gray-600">Selectionnez une file d'attente pour traiter les contacts sur le terrain</p>
        </div>

        {!selectedQueue && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {QUEUE_BLOCKS.map((block) => {
          ...
          <div className="space-y-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className={`${selectedQueueBlock?.bgColor ?? 'bg-white'} border-2 ${selectedQueueBlock?.borderColor ?? 'border-gray-200'} rounded-xl p-4 sm:p-5 w-full`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-gray-900">Terrain · {selectedQueueBlock?.title}</h2>
                    <p className="text-sm text-gray-600">{selectedQueueBlock?.subtitle}</p>
                  </div>
                </div>
              </div>
              <div className="w-full lg:w-auto flex flex-col gap-2">
                <Button variant="outline" onClick={() => setSelectedQueue(null)} className="w-full lg:w-auto">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-1">
                  <label className="text-sm font-semibold text-gray-900 whitespace-nowrap">Campagne:</label>
                  <CampaignSelect
                    value={selectedCampaign}
                    onValueChange={setSelectedCampaign}
                    className="w-full sm:w-64"
                    typeCampagne="avec_action_terrain"
                  />
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-1">
                  <label className="text-sm font-semibold text-gray-900 whitespace-nowrap">Gestionnaire:</label>
                  <Select
            ...
            {error && (
              <div className="bg-red-50 border-l-4 border-red-500 rounded p-4">
                <p className="text-sm text-red-900">Erreur lors du chargement du contact: {error.message}</p>
              </div>
            )}
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    <div className="flex flex-col gap-4">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="text-xs text-gray-500">File Terrain · {selectedQueueBlock?.title}</div>
                      </div>

                      {(contactSummary.name || contactSummary.phone || contactSummary.email || contactSummary.address || contactSummary.campaignId || contactSummary.accountId) && (
                        <div className="rounded-xl border border-gray-100 bg-slate-50 p-3">
                          {contactSummary.name && (
                            <div className="text-sm font-semibold text-gray-900 mb-2">{contactSummary.name}</div>
                          )}
