...
export function useCampagnesData() {
  ...
  const campaigns: Campaign[] = apiCampaigns.map((apiCampaign) => {
    ...
    // Get full model data for workflow visualization
    // Match by model ID since queries are deduplicated by queryKey
    const modelQueryResult = relatedDataQueries.find(
      (q) => q.data?.type === 'model' && q.data?.data &&
             (q.data.data as ModeleAPIResponse).id_modele === apiCampaign.id_modele
    );
    const modelData = modelQueryResult?.data?.data as ModeleAPIResponse | undefined;

    return {
      id: apiCampaign.id_campagne,
      code: apiCampaign.id_campagne, // Use API code directly
      title: apiCampaign.nom_campagne,
      description: apiCampaign.description || '',
      image: '', // Will be replaced by workflow visualization
      status,
      statusColor,
      ...
      modelData: modelData ? {
        listeAction: modelData.liste_action || '',
        variableCible: modelData.variable_cible,
        objectif: modelData.objectif,
      } : undefined,
