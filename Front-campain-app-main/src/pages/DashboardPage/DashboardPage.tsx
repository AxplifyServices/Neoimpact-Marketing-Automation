...
import { Phone, Target, Activity, Filter, AlertCircle } from 'lucide-react';
import {
  BarChart,
  Bar,
  Funnel,
...
export default function DashboardPage() {
  ...
  console.log('[Dashboard] Has graph?', !!dashboardData?.graph);
  console.log('[Dashboard] Graph object:', dashboardData?.graph);
  ...
  const handleSelectionChange = useCallback((_campaigns: string[], states: string[]) => {
    // Only states trigger filter updates (campaigns are filtered by states)
    setSelectedStates(states);
    setSelectedCanal(null);
  }, []);

  // Transform graph data to workflow blocks
  const workflowBlocks = useMemo(() => {
    if (!dashboardData?.graph) {
      console.log('[Dashboard] No graph data available');
      return [];
    }

    const { nodes, edges } = dashboardData.graph;
    console.log('[Dashboard] Graph data:', { nodes, edges, campaign_id: dashboardData.graph.campaign_id });

    const blocks = nodes.map((node) => {
      // Find parent by looking at edges where this node is the target
      const parentEdge = edges.find(edge => edge.to === node.id);

      return {
        id: node.id,
        canal: node.canal,
        delai: 0,
        parentBlockId: parentEdge ? parentEdge.from : null,
        objet: node.action || '',
        contenu: `${node.count} actions`,
        conditions: [],
      };
    });

    console.log('[Dashboard] Transformed blocks:', blocks);
    return blocks;
  }, [dashboardData?.graph]);

  const getBlockDisplayNumber = useCallback((blockId: string) => {
  ...
  const conversionFunnelData = useMemo(() => {
    ...
    return [
      ...
      { name: 'Conversion', value: kpis.closing_total, fill: '#f59e0b' },
    ];
  }, [dashboardData]);
  ...
  const funnelChartData = useMemo(() => {
    if (!dashboardData?.series.funnel_by_id_action) return [];
    return dashboardData.series.funnel_by_id_action.map(item => ({
      name: item.ID_Action,
        ...
        <>
          ...
          {funnelChartData.length > 0 && (
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-8">
              <h2 className="text-xl font-bold text-gray-900 mb-6">Funnel d'avancement</h2>
