...
export function DataTableColumnHeader<TData, TValue>({
  ...
  return (
    <div className={cn('flex items-center space-x-2', className)} aria-sort={ariaSort as any}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant='ghost'
            size='sm'
            className='data-[state=open]:bg-gray-100 h-8'
            aria-label={`Sort by ${title}${sortDirection ? ` (currently ${sortDirection === 'asc' ? 'ascending' : 'descending'})` : ''}`}
          >
            <span>{title}</span>
            {sortDirection === 'desc' ? (
